...
try {
    ...
    echo 'Lower long   (ATM-2) - Trading symbol : ' . $lowerLong->getTradingSymbol() . PHP_EOL;
    ...
    echo 'Middle short put order placed successfully. Order ID: ' . print_r($response2, true) . PHP_EOL;

    // Step 4: Buy the ATM-2 put
    $body3 = new \Upstox\Client\Model\PlaceOrderV3Request();
    $body3->setInstrumentToken($lowerLong->getInstrumentKey());
    $body3->setQuantity($lowerLong->getLotSize());
    ...
    echo 'Lower long put order placed successfully. Order ID: ' . print_r($response3, true) . PHP_EOL;
