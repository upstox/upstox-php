...
Ready-to-run PHP examples for common Nifty 50 options strategies using the [Upstox PHP SDK](https://packagist.org/packages/upstox/upstox-php-sdk).

Each script finds the required option legs via `InstrumentsApi->searchInstrument()` and places market orders via `OrderApiV3->placeOrder()`. Replace `your_access_token_here` with your token before running.
